...
unless ( $opts{ no_check } ) {
    open( my $gfh, '>', $OUTPUT . '/genomes.list' ) || die "Can't open $OUTPUT/genomes.list for writing: $!\n";
...
unless ( $opts{ no_check } ) {
    ...
    my $cmd = "$CHECK_ATT_FILES_EXEC -d $OUTPUT/genomes.list -a $OUTPUT -f $OUTPUT --resolve";
    if ( $opts{ nuc } ) {
        $cmd .= " --nuc";
    }
